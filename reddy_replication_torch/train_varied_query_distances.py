...
def generate_input_seqs_TI_fixed(mus_label, mus_class, labels_class, S, N, Nmax, eps=0.1, B=0, p_B=0, P=None, p_C=0,
    ...
    if random_query:
        ...
        # Determine the correct label based on positions
        # If pos[0] < pos[1], then item1 < item2 (label from second half)
        # If pos[0] > pos[1], then item1 > item2 (label from first half)
        targets_c_ind = None  # We'll handle labels differently below
    ...
    for s in range(S):
        if not filt_C[s]:
            if random_query:
                # Original behavior: label comes from the context
                labels[s, label_choices_c[s, targets_c_ind[s]]] = True
            else:
                # New behavior: determine label based on query positions
                pos1, pos2 = query_pos[0][s], query_pos[1][s]
                if pos1 < pos2:
                    # item1 < item2, use label from second half (L//2 to L)
                    labels[s, label_mapping_index[s, 0] + L // 2] = True
                else:
                    # item1 > item2, use label from first half (0 to L//2)
...
def sample_query_positions_by_distance(batch_size, n_items=7):
    ...
    Returns tuple of two arrays (pos1, pos2) each of length batch_size,
    where each pair contains two different positions from the n_items chain.
    """
    ...
    for i in range(batch_size):
        ...
        # Sample pos2 != pos1 to ensure valid pair
...
def main_varied_query():
    """Main training function with varied query distances."""

    # Create modified config for varied query distance training
    cfg = dd(dict(
        model=dd(dict(
            h_dim=128,
            n_heads=8,
    ...
    experiment_name = 'TI_varied_query_I{}_K{}_N{}_L{}_D{}_a{}_B{}_pB{}_pC{}_eps{}_lr{}_drop{}_custom_ln{}_wDecay{}_{}'.format(
        cfg.train.niters,
        cfg.data.K,
        cfg.seq.N,
        cfg.data.L,
    ...
    # Test set with random query positions
    ...
    if cfg.train.optim == 'adam':
        ...
        optimizer = optim.SGD(model.parameters(), lr=cfg.train.learning_rate, weight_decay=cfg.train.w_decay)
    ...
    for n in range(cfg.train.niters):
        ...
        # Sample random query positions for this batch
        query_positions = sample_query_positions_by_distance(cfg.train.batch_size, n_items=7)

        # Generate batch with specified query positions
            ...
            with torch.no_grad():
                ...
                if cfg.log_to_wandb:
                    ...
                    if cfg.save_weights:
                        ...
                        ax1.imshow(out_dict_TI['block_0']['weights'].cpu().mean(axis=0)[0], cmap=ATTENTION_CMAP)
                        wandb.log({'l0_attn_map_ti': fig1, 'iter': n})
                        fig2, ax2 = plt.subplots()
                        ax2.imshow(out_dict_TI['block_1']['weights'].cpu().mean(axis=0)[0], cmap=ATTENTION_CMAP)
                        wandb.log({'l1_attn_map_ti': fig2, 'iter': n})
                        plt.close('all')

                # Calculate induction strength on TI
                induction_strength_per_head = calculate_induction_strength(cfg, test_inputs_TI, n, out_dict_TI)
                if cfg.log_to_wandb:
                    for h in range(cfg.model.n_heads):
                ...
                print(f'iter {n}, loss: {loss.item():.4f}, ti_accuracy: {ti_accuracy.item():.4f}, '
                      f'ic_accuracy: {icl_accuracy.item():.4f}, iw_accuracy: {iwl_accuracy.item():.4f}')

        if cfg.save_model and n % cfg.saving_interval == 0 and n > 0:
            if not os.path.exists(cfg.model_dir):
                os.makedirs(cfg.model_dir)
            file_path = os.path.join(cfg.model_dir, experiment_name)
